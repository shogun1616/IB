...
if st.button("Ejecutar análisis y ajustar modelo"):
    if df is None:
        ...
        plot_df = pd.merge_asof(df_plot.sort_values("hora"), df_plot_obs.sort_values("hora"), on="hora", direction="nearest")
        plot_df = plot_df.set_index(pd.to_timedelta(plot_df["hora"], unit='h'))
        st.subheader("Gráfica del modelo y puntos observados")
